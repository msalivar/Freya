...
public class CRUD {
    ...
    public static class writeMessage extends AsyncTask<MainActivity.AsyncParams, Void, returnParam>
    {
        ...
        protected returnParam doInBackground(MainActivity.AsyncParams... params) {
            ...
            try {
                ...
                out.write(params[0].unsynced.toString());
                out.close();
        ...
        protected void onPostExecute (returnParam result){
            if (result.response == 200)
            {
                ...
                /*if (getInfo.unsynced != null)
                    for (int i =0; i < getInfo.unsynced.length(); i++)
                  */
    ...
    public static class deleteMessage extends AsyncTask<String, Void, returnParam>
    {

        @Override
        protected returnParam doInBackground(String... params) {
            URL url = null;
            HttpURLConnection urlConnection = null;
            // URL is attached to theunique ID to identify which part of the JSON needs to be deleted
            String test = "https://sensor.nevada.edu/GS/Services/edge/" + params [0] + "/";
            try {
                // connect to URL
                url = new URL(test);
                urlConnection = (HttpURLConnection) url.openConnection();
